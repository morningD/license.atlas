...
**历史线程折叠（2026-09-06，KB build 第 4 步）**：provisional 门放开裸 subject 线程后，`findOwningEntry` 按显著词重叠把未覆盖线程并入既有 curated/API 条目（CAL beta/UPL/EUPL/OFL/CERN OHL 等完整史），真孤儿才新建 provisional。**规模警告**：一次大 fold ≈ 900 条 point 提取（125 批、45 分钟）+ ~40 条裁决重跑（30 分钟）——必须 nohup 后台跑，前台必被 shell 超时杀。fold 后数据流顺序（错一步就拿到污染数据）：extract-full-bodies 刷包 → extract-all-points → enrich → update:tracker（内部 prepare→local 裁决→apply）→ **apply 重放后 v2 才是裁决终态**（prepare 会把 status 重置回规则值，恢复/手改 v2 后必须先 apply 再给任何下游消费）。

**编排器自动化**（2026-08-23，`update-tracker.mjs`）：**无人值守裁决（2026-09-04 起）**——灰色地带不再卡人：run/apply 对矛盾证据或低置信的终态判断**保守降级为规则状态 rule_status**（非一律 pending——fold 历史线程曾把 OSI 已批准条目降成 pending；矛盾记入 conflicts、rationale 加 `[conservative fallback]` 前缀），verify/test-tracker-data 只对"终态仍带矛盾/低置信"报 critical，board_vote/withdrawal 期望降为 warning；`tracker-manual-baseline.json` 基线门与 `--rebaseline` 已退役，manual-review.json 仅作审计（`reportManualReviewSoft` 软报告）；point coverage 失败非阻塞（UI snippet 兜底 + 下轮重试）；verify 后 `resync-baseline-batch.mjs` 把 outputs 归一单基线批（防 runner LLM 批抢占）。旧机制：invalid/missing 裁决自动 LLM 重跑最多 3 轮（key 缺失自动读 opencode 配置）；KB run 脚本对 "conflicts 未置 requires_manual_review" 的输出直接矫正。`--strict-manual-pending` 恢复任何 blocking 即失败的旧模式。**编排器步骤**：邮件抓取 → apply-llm-batches → build → **3.5 point 提取**（extract-full-bodies 刷包 + extract-all-points 增量，LLM env 注入——没有这步新邮件会卡 coverage 门）→ enrich → **3.6 裁决状态恢复**（apply 幂等重放 outputs）→ prepare/裁决/verify → 基线检查 → 质量门 → sync。2026-09-01 起稳态：全链路 ~3 分钟、零 LLM（除非有新邮件/新矛盾）。**point 回填**：enrich 加载 all-points-manifest 且优先于 build 的 clean_body 兜底（point/point_zh/sentiment），新邮件 point 同轮落位——point 合并只在 build 做，无回填则新邮件 point 滞后一轮且泄漏原文。
