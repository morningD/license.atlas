...
1. 运行 `npm run update:tracker`（KB 路径默认 ../KB）。LLM 环境变量：设 ANTHROPIC_API_KEY/ANTHROPIC_AUTH_TOKEN=key（缺失时读 /Users/momo/.config/opencode/opencode.json 的 zhipuai-coding-plan）、ADJUDICATION_OPENAI_BASE_URL=https://open.bigmodel.cn/api/coding/paas/v4（裁决走 OpenAI 兼容单次调用通道，快且稳）、ADJUDICATION_BATCH=4、ADJUDICATION_CONCURRENCY=2（golden 测试集验证 3.9× 提速且质量无损；并发勿超 2，为其他 session 预留 Pro 套餐余额）、STATUS_ADJUDICATION_MODEL=glm-4.6、POINTS_MODEL=glm-4.6。编排器已内嵌：灰色地带 manual review 按 scripts/tracker-manual-baseline.json 放行，invalid/missing 裁决条目自动用 LLM 重跑（最多 3 轮）。若报 "still invalid after 3 LLM retries" 或 "New manual-review item(s) outside the known baseline"：不 push，在日志记录残留/新增 ids 后结束（新基线条目留人工 review，人工确认后由维护者跑 `npm run update:tracker -- --rebaseline` 采纳）。
