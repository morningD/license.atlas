...
**人工基线输出批**（2026-09-01）：outputs 目录只保留一个全量人工基线批 `batch-207-status-free-hash.out.jsonl`（194 条 agent-eligible 条目，final_status 取自最后推送的 tracker），此前所有 LLM 输出批已退役。**outputs 只允许存在这一个批文件**——runner 的自动重裁决会写出自己的 batch-N，apply/verify 的 loadOutputs 按"hash 优先、同 hash 取最后行"选择，LLM 行会静默抢占基线行并把 44 条翻成 manual review（2026-09-02 实测）；runner 轮结束后若 outputs 出现额外批文件必须与基线批合并或退役。人工裁决记录：`unicode-dfs-2016 = legacy`（2018-09-04 board 邮件明载 "granted Legacy Approval"，OSI API 的 legacy 标志位滞后）、`public-benefit-zero-copyright = withdrawn`（2025-01-22 官方撤回邮件，curated 记录滞后）、`python-2-0 = legacy`（2026-06-29 board vote 批准 Python-2.0.1 as Legacy，tracker 单条目承载整个家族终态，superseded 无承接对象）。维护规则：人工裁决变更后直接改 v2 + 重新 prepare + 按新 hash 重新合成基线批（**合并保留现有行，不要覆盖**——覆盖会丢 hash 行导致相邻条目 critical），不要留多个 hash 世代混杂的批——apply 在 exact hash 匹配失败时会 fallback 到"最后一个副本"，陈旧批会静默覆盖人工值。
...
**全文提取与立场判型**（2026-09-02，006237 事件）：extract-all-points 曾把 clean_body 截断到 1500 字符——26% 事件被截，而**长邮件的立场转折恰恰在后半部**（先复述客套后表态），006237（McCoy Smith）为 OpenMDW-1.1 辩护的整段论证都在截断线之后，被误判 oppose。现改为：`MAX_BODY_CHARS=16000`（p99=12.9K，覆盖 99.4%）+ 贪心字符预算分批（`BATCH_CHAR_BUDGET=50000`，防超长 prompt）+ 超限邮件单发全文 + `POINTS_CONCURRENCY` worker pool（默认 2）。prompt 加两条判型规则：①被剥离引用造成的悬空指代（"the example above"）不得当作发言者自己引入的例子；②sentiment 只针对**被审许可证**——批评先例许可证（IPL/ModelGo）或为被审许可证反驳批评者都不是 oppose（辩护=support）。存量 966 条截断条目已全部重提取并压缩（3671 条 point、zh 全覆盖、>300 字符归零），状态经基线重合成闭环重新锚定。
